...
# This script builds the Docker image for admin-portal
...
# Copy ONLY the admin-portal app
mkdir -p "$TEMP_CONTEXT/apps"
cp -R apps/admin-portal "$TEMP_CONTEXT/apps/"

# Build the Docker image from the temporary context
# Pointing to the Dockerfile inside the admin app folder
docker build -t "$IMAGE_NAME" -f "$TEMP_CONTEXT/apps/admin-portal/Dockerfile" "$TEMP_CONTEXT"
